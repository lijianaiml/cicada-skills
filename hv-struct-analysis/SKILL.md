...
description: |
  ...
  触发信号是请求的性质，而非具体用词：用户期望的是全面调研（来龙去脉、横向对比、发展趋势、多方信息综合），而不是一个快速答案。典型场景：用户明确表达「深度/系统/多维度」研究意图，如："深度研究/分析/调研"、"系统性梳理"、"纵横分析"、"竞品分析报告"，只要上下文暗示需要系统性的深度研究（而非简单的概念解释），都应该触发。不要用于简单的名词解释（用户只是问"XX是什么"），用户补充了明确的简短输出要求（"简单说一下"、"一句话概括"）。
...
**项目名规则**：使用概念的简洁中文或英文名称，不含空格，例如 `量子纠缠分析`、`blockchain-analysis`。
...
- **子Agent 1 — 纵向信息**：研究对象的起源、创始人背景、发展历程、关键事件、版本迭代、融资、战略转向、危机；信息记录到research/`纵向信息.md`。
- **子Agent 2 — 横向信息**：竞品识别、各竞品的特点和用户口碑、行业对比评测、市场份额；信息记录到research/`横向信息.md`。
- **子Agent 3**（复杂对象才需要）：补充信息，如创始人深度背景、行业环境变化、用户社区讨论（GitHub issues、Reddit、Twitter/X、知乎等）；信息记录到research/`补充信息.md`。
- **子Agent 4**：复核research/中的事实和来源，有问题追加到 `issues.md`，没问题追加到 `sources.md`。
...
> - 如果用户环境中安装了 web-access skill（检查路径 `~/.claude/skills/web-access/SKILL.md` 是否存在），优先加载它并遵循其指引，它提供更强的浏览器CDP能力
...
- [ ] 没有触犯绝对禁区里的任何一条？
- [ ] `sources.md` 中标记为 ❌ 的信息不得出现在 `[研究对象名称]_横纵分析报告.md` 中？
- [ ] `sources.md` 中标记为 ⚠️ 的信息在报告中需明确标注不确定性？
- [ ] `issues.md` 中仍为 [ ] 的未解决问题，若涉及核心结论，须在报告"研究说明"部分注明局限？
- [ ] PDF排版美观、结构清晰、可读性好？
