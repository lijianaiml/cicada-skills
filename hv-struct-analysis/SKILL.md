---
name: hv-struct-analysis
description: |
  横纵分析法（Horizontal-Vertical Analysis）深度研究Skill。
  ...
  触发词包括但不限于："深度研究/分析/调研"、"系统性梳理"、"纵横分析"、"竞品分析报告"、"帮我吃透/摸清楚"、"产业链分析"、"技术路线梳理"、"全方位评估"。也适用于用户丢来一个产品名、公司名、技术名词说"帮我研究一下这个"的场景。
  不要用于简单的名词解释（用户只是问"XX是什么"）、用户补充了明确的简短输出要求（"简单说一下"、"一句话概括"）。
...
> **方法论溯源**
> 横纵分析法由数字生命卡兹克（Khazix）提出，经过cicada修改。融合了语言学中的历时-共时分析（Saussure）、社会科学中的纵向-横截面研究设计、商学院案例研究法、以及竞争战略分析的核心思想，形成了一套适用于产品/公司/概念/人物的通用研究框架。核心原则不变：纵向追时间深度，横向追同期广度，最终交汇出判断。
...
<研究对象>/
├── sources.md         # 信息来源核验表
├── issues.md          # 研究过程中发现的问题与疑点
├── research/
│   ├── 横向信息.md    # 同类/关联概念的横向对比与分布
│   ├── 纵向信息.md    # 该概念的历史演变与深度剖析
│   └── 补充信息.md    # 边缘信息、争议、特例、延伸话题
├── [研究对象]_横纵分析报告.md          # 最终分析报告（Markdown）
└── [研究对象]_横纵分析报告.pdf         # 最终分析报告（PDF）
```
...
这个方法论的质量完全取决于信息的丰富度和准确性。**必须联网搜索**，不能仅靠已有知识。研究报告的价值在于深度和完整度，所以信息收集阶段宁可多搜，不要因为信息不够导致后面的分析浮于表面。收集到的信息必须加以核验保证准确性。

### 多 Agent 收集策略

#### 文件说明
- `sources.md`：核验中发现的问题来源汇总（❌ 不可用条目 + 重要 ⚠️ 条目），供报告撰写时排除和标注
- `issues.md`：研究过程中发现的矛盾、缺口、疑点，供撰写报告时说明局限性

#### 整体架构

Orchestrator 负责调度，共启动 6 个子 Agent：

```
Orchestrator
├── [并行启动]
│   ├── Agent-横向收集 → research/横向信息.md（含行内脚注来源）
│   ├── Agent-纵向收集 → research/纵向信息.md（含行内脚注来源）
│   └── Agent-补充收集 → research/补充信息.md（含行内脚注来源）
│
├── [各自完成后立即触发，由 Orchestrator 触发而非收集 Agent 自行触发]
│   ├── Agent-横向核验 → 追加写入 sources.md + issues.md（前缀 [横向]）
│   ├── Agent-纵向核验 → 追加写入 sources.md + issues.md（前缀 [纵向]）
│   └── Agent-补充核验 → 追加写入 sources.md + issues.md（前缀 [补充]）
│
└── [六个 Agent 全部完成后]
    └── Orchestrator 按第二步至第五步执行分析与报告生成
```

#### 执行步骤

**Step 1：Orchestrator 并行启动三个收集 Agent**

同时启动，互不等待：

- Agent-横向收集
  - 输入：分析主题 + 横向维度定义（竞品识别、各竞品的特点和用户口碑、行业对比评测、市场份额）
  - 输出：`research/横向信息.md`（正文含行内脚注来源，不确定或单一来源的信息在行末加 `⚠️` 标记）

- Agent-纵向收集
  - 输入：分析主题 + 纵向维度定义（研究对象的起源、创始人背景、发展历程、关键事件、版本迭代、融资、战略转向、危机）
  - 输出：`research/纵向信息.md`（同上）

- Agent-补充收集
  - 输入：分析主题 + 补充维度定义（创始人深度背景、行业环境变化、用户社区讨论（GitHub Issues、Reddit、Twitter/X、知乎等））
  - 输出：`research/补充信息.md`（同上）

**收集 Agent 约束：**
- 所有事实陈述必须附有可点击的来源链接（以脚注形式内嵌）
- 无法确认来源或仅有单一来源的信息，行末标注 ⚠️

**收集 Agent 联网指引（写入每个收集 Agent 的 prompt）：**
- 搜索策略：先用 WebSearch 发现信息来源和线索，找到具体 URL 后用 WebFetch 深入提取；多关键词组合，不要只搜一次就放弃
- 若环境中存在 `~/.claude/skills/web-access/SKILL.md`，优先加载并遵循其指引
- 如研究对象涉及学术概念、算法、AI 模型、技术范式等，必须查询 arxiv：
  ```
  curl -s "https://export.arxiv.org/api/query?search_query=all:关键词1+AND+all:关键词2&max_results=10"
  ```
  返回 XML 含标题、作者、摘要、发布日期、PDF 链接；找到关键论文后用 WebFetch 访问 `https://arxiv.org/abs/论文ID` 获取详情

**Step 2：哪个收集 Agent 完成，Orchestrator 立即启动对应核验 Agent**

- Agent-横向核验
  - 触发条件：`research/横向信息.md` 写入完成
  - 输入：`research/横向信息.md` 完整内容
  - 输出：以 `[横向]` 为前缀，追加写入 `sources.md` 和 `issues.md`

- Agent-纵向核验
  - 触发条件：`research/纵向信息.md` 写入完成
  - 输入：`research/纵向信息.md` 完整内容
  - 输出：以 `[纵向]` 为前缀，追加写入 `sources.md` 和 `issues.md`

- Agent-补充核验
  - 触发条件：`research/补充信息.md` 写入完成
  - 输入：`research/补充信息.md` 完整内容
  - 输出：以 `[补充]` 为前缀，追加写入 `sources.md` 和 `issues.md`

**核验 Agent 约束：**
- 必须逐条处理每个脚注，不得跳过；收集 Agent 标注了 ⚠️ 的信息优先处理
- 核验结果直接在对应 research 文件的脚注末尾追加状态标记：
  - `✅` 已通过交叉核验
  - `⚠️ [原因]` 单一来源或可信度存疑
  - `❌ [原因]` 来源不可访问、内容与摘要严重不符或来源明显不可靠

  示例：
  ```
  [^1]: 作者, 论文名, URL ✅
  [^2]: 某博客, URL ⚠️ 单一来源，无第二佐证
  [^3]: 某网站, URL ❌ URL无法访问
  ```
- 将所有 ❌ 条目和重要 ⚠️ 条目以 `[维度]` 为前缀追加写入 `sources.md`；若不存在则自行创建：
  ```
  ## [横向] 问题来源
  - ❌ [^3]: 某网站 — URL无法访问，对应正文信息不可用
  - ⚠️ [^2]: 某博客 — 单一来源，涉及核心结论需在报告中标注不确定性
  ```
- 发现的矛盾、缺口、疑点追加写入 `issues.md`（前缀同上）；若不存在则自行创建

**核验 Agent 联网指引（写入每个核验 Agent 的 prompt）：**
- 使用 WebFetch 直接访问收集 Agent 提供的来源 URL，确认内容真实存在且与摘要一致
- 使用 WebSearch 交叉验证关键事实，寻找第二来源佐证
- 不要发散搜索新信息，核验范围严格限定在当前信息文件的脚注来源内
- 若环境中存在 `~/.claude/skills/web-access/SKILL.md`，优先加载并遵循其指引

**Step 3：Orchestrator 门控检查**

六个 Agent 全部完成后，Orchestrator 执行以下检查，全部通过才进入报告生成：

- `research/横向信息.md`、`research/纵向信息.md`、`research/补充信息.md` 中每个脚注都已标注状态（✅ / ⚠️ / ❌）
- `sources.md` 存在且包含 `[横向]`、`[纵向]`、`[补充]` 三个维度的内容

任一条件不满足，重新触发对应维度的核验 Agent，不得跳过直接生成报告。

检查通过后，按第二步至第五步执行分析与报告生成。
...
3. **运行转换脚本**：
   ```bash
   python [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "研究对象名称" --author "cicada"
   ```
...
- [ ] 没有触犯绝对禁区里的任何一条？
- [ ] 报告中所有事实陈述均来自脚注标记为 ✅ 的信息；标记为 ❌ 的信息不得引用
- [ ] 脚注标记为 ⚠️ 的信息若出现在报告中，行文中需明确标注不确定性（如"据单一来源显示"、"尚未得到交叉确认"）
- [ ] `issues.md` 中仍为 `[ ]` 的未解决问题，若涉及报告核心结论，须在"五、信息来源"章节末尾以"已知局限"小节列出
- [ ] PDF排版美观、结构清晰、可读性好？
